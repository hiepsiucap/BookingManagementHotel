...
export const CreateABooking = async(formdata : FormData) : Promise<any>=>{
  ...
  const special_request=formdata.get("special_request") as string;
 ...
 try{
  ...
  const isAvai=  roomTypes.bookings.filter((booking)=>{
            ...
            if(isAvai.length>0) return {success: false , message: "Phòng bạn đặt đã hết", reset: true}
    ...
    let totalprice=0;
     if(TypeRent=="priceHour")
           {
              if(Number(startHour) <22 && Number(startHour) +Number(numbHour)> 22 ) totalprice= price?.priceNight+price?.priceHour*(Math.ceil(Number(startHour) +Number(numbHour)- 22))
             else if(Number(startHour)> 22 || Number(startHour)<6 )
                { 
                    if((Number(numbHour)+ Number(startHour)%24)>12 && Number(startHour)<22)  totalprice= price?.priceNight+price.priceHour*(Math.ceil(Number(numbHour)+ Number(startHour)-12))
          ...
          else if(TypeRent =="priceNight")
           {
            ...
            return totalprice= (Math.ceil(22-Number(startHour)))*price.priceHour+price.priceNight;
           ...
           else if(TypeRent =='priceDay')
           {
             if(Number(startHour)>22|| Number(startHour)<6) 
                totalprice= price.priceNight+price.priceDay* (Number(numbDay)-1)
           else if(Number(startHour)<14)  return totalprice= (Math.ceil(14-Number(startHour)))*price.priceHour+price.priceDay* Number(numbDay)
            else totalprice=price.priceDay* (Number(numbDay))}
          console.log(total_price, totalprice)
      if(Number(total_price) !=totalprice) return {success: false , message: "Phòng bạn đã được cập nhật giá ", }
      if(rentalid)
      {
